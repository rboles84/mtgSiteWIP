...
async function validateIdentityFixtures({ key, rawId, claimsFile, provenance, ledgerRow }) {
  const errors = [];
  ...
  for (const fixture of fixtures) {
    ...
    for (const evidenceId of fixture.evidence_claim_ids || []) {
      ...
      else if (inferSemanticRole(claim) !== "substantive_claim") errors.push(`${key} ${fixture.fixture_id}: evidence claim ${evidenceId} is not substantive`);
    }
    if (fixture.fixture_type === "provenance") {
      ...
      if (!match) errors.push(`${key} ${fixture.fixture_id}: generated provenance entry is missing`);
        ...
        for (const evidenceId of fixture.evidence_claim_ids || []) {
          if (!match.evidence_claim_ids.includes(evidenceId)) errors.push(`${key} ${fixture.fixture_id}: provenance omits evidence claim ${evidenceId}`);
        }
      }
    }
  }
