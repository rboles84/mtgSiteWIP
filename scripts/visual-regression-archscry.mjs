...
const captureConfigs = [
  ...
  { name: "dossier-mobile", width: 375, height: 2200, state: "dossier" },
  { name: "dossier-tablet", width: 768, height: 2200, state: "dossier" },
  { name: "dossier-desktop", width: 1440, height: 2200, state: "dossier" },
];
...
async function waitForDossier(page) {
  await page.waitForFunction(() => {
    ...
    const axisList = document.querySelector(".vm-axis-list");
    const adjacentCards = document.querySelectorAll(".adjacent-card");
    return Boolean(
      result &&
      !result.classList.contains("hidden") &&
      name &&
      ...
      axisList &&
      axisList.textContent &&
      axisList.textContent.trim().length > 0 &&
