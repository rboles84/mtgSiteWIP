...
|---|---|---|---|
| `/` | `index.html` | Canonical home gateway and Identity Signal showcase. | `assets/js/shared/vm-radar.js`, `assets/js/home/home.js` with lazy `assets/js/vendor/chart.umd.js`, `assets/js/shared/reduce-motion.js`, `assets/js/shared/vm-topbar.js` |
| `/archscry/` | `archscry/index.html` | Placement experience: landing, quick adaptive reading, dossier result, archived Scrying Terminal. | `assets/js/vendor/chart.umd.js`, `assets/js/shared/vm-radar.js`, `assets/js/archscry/index.js`, `assets/js/shared/shared.js`, `assets/js/archscry/adaptive-placement.js` |
...
| Home route | `assets/css/home.css`, `assets/js/home/home.js`, `index.html` | Canonical home shell styling and behavior extracted from the former preview-page inline CSS/JS while still reusing shared tokens, layout, topbar, local `graph.js`, `data/identity-layers.json` preview metadata, and `data/factions.json` lore notes. |
...
|---|---|---|
| `assets/js/archscry/index.js` | ES module loaded by `archscry/index.html` | Imports `adaptive-placement.js`, exposes inline HTML handlers onto `window`, then initializes on `DOMContentLoaded`. |
| `assets/js/shared/shared.js` | Classic script loaded before placement modules | Creates global session helpers, Supabase client access, storage helpers, interview calls, save/resume/sign-out flows. |
...
| Faction artifact builder | `scripts/build/build-faction-artifacts.mjs` | Regenerates display metadata, adaptive model, schema, and Supabase faction context from raw factions plus identity layers. |
...
| Surface | Where | Shape |
|---|---|---|
| Placement result | `docs/reference/data-contracts.md`, `assets/js/archscry/adaptive-placement.js`, `assets/js/shared/shared.js`, edge function | Versioned object with legacy faction fields plus layered `identity`, confidence, mana scores, matches, starter profile, evidence, and stage history. |
| Browser globals | `assets/js/shared/shared.js`, `assets/js/archscry/index.js`, `assets/js/maze/research-init.js` | Inline handler functions, `VM_SESSION`, `vm_*` helpers, and research handlers. |
| Supabase profile row | `assets/js/shared/shared.js` | Compatibility fields plus richer `placement_result` source of truth. |
| Scryfall API calls | `assets/js/maze/research-search.js`, `assets/js/archscry/index.js` | Search/exact/random endpoints for Maze and card-art enrichment. |
