...
VM-022 adds a Maze-first query contract in `docs/contracts/maze-query-contract.md` and the first reusable core surface in `research/maze-query-core.js`. The core owns request/result normalization, raw syntax cleanup, format application, parser-mode classification, source-context normalization, and path-entry generation. The primary `doSearch()` route path now builds a `MazeQueryRequest` and consumes `resolveMazeQueryRequest()`, while the route adapter still owns DOM state, storage, Scryfall fetch execution, Query Inspector rendering, modal behavior, stash behavior, and boot sequencing.
