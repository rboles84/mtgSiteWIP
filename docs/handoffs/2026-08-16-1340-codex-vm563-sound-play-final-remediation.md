...
- Dune owner-correction producer/test updates and the governed Archscry media projection required by the exact-printing replacement
...
- Changed 18 tile fields and 92 modal fields across the final corpus.
- Retained every identity, slot, order, and every Sound/Play card/relationship/printing except the one owner-authorized Dune Sound replacement.
- Replaced Dune / Scour from Existence with Dune-Brood Nephilim, exact printing `15b4ee44-28c4-4a39-9c06-aca43787954f`, while preserving Scour's VM-561 record and both VM-558/VM-563 decision-history entries.
...
- Witch/Amphin retains its card under narrower approved Vox Mana facets; no missing-color psychology was manufactured.
- Owner rendered review rejected Dune/Scour. Dune-Brood Nephilim replaces it under `dune_claim_0004`, `dune_claim_0005`, DUNE-EVID-004/007/009, and the committed exact Scryfall printing/rules record. It is a bounded card anchor, not an official faction or doctrine.
- The earlier VM-558 rejection of a generic Dune-Brood swarm bridge remains recorded. The corrected bridge depends on the governed card anchor, exact “nameless thousands” voice, and land-scaled Sand-token action together; it does not use color identity or generic token/combat language as proof.
- Remaining hard owner blockers and modal-content-model reviews are both zero.
...
- The Dune-Brood relationship remains Vox Mana synthesis at its governed altitude; it must not be treated as an official universal four-color doctrine.
...
- `git diff --check` — PASS.
- `node research/apply-vm563-sound-play-remediation.mjs` — PASS, byte-stable check mode; one relationship replacement and 37/119/73/46/4 preserved.
- `npm.cmd run scryfall:index:check` — PASS, byte-identical governed media projection with 1,178 occurrences / 572 resolver keys / 37 identities.
- `npm.cmd run test:vm559-media-projection` and `npm.cmd run test:vm559-resolution` — PASS.
- Focused Dune desktop and mobile replay — PASS; exact Dune-Brood quote/modal/image, real desktop hover, focus restoration, and viewport bounds verified.
- In-app rendered Dune review — PASS; full card image, modal wrapping, exact modal copy, Scryfall printing link, Escape close, and trigger focus restoration verified.
- Workbook comparison against `f5ede39a7f03caf6c0644c80142c201643605b85` — PASS; exactly 37 sheets and only `33 DUNE Dune - Aggression` changed.
...
- Protected behavior: all unrelated Archscry sections, placement/scoring/qualification, routing, result states, every non-Dune card selection/printing/order, identity semantics, VM-559 media policy, and canon corpus.
...
- Suggested owner command: `npm.cmd run review:vm559 -- --identity=DUNE --viewport=desktop`.

## Not touched

Start Here, Matrix, Card Signals, Mana Notes and its tiers, commander/precon recommendations, dossier layout, hover implementation, placement/scoring/qualification, routing, result states, certified identity semantics, Glint and every other accepted Sound/Play relationship, `docs/research/canon/`, VM-559 acceptance/deployment state, and production hosting.

## Follow-up recommendations

Run the one deterministic Dune desktop review command and judge Dune-Brood beside Aurelia, the modal value, and wrapping. If accepted, proceed through the existing VM-559 integration/deployment closeout; if a concrete defect is found, report that exact row and field.
