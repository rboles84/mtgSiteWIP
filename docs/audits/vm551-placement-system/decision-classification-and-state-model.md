...
| Weak evidence | Still returns a primary after 6–8 forced answers. | No insufficient-evidence state; corrected analysis finds 2,901 primary paths below the generated strong-hit minimum proxy, while zero paths are genuinely negative-only. | Return insufficient evidence or a broad family hypothesis. |
