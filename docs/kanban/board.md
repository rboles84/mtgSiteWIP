...
- [VM-517 - White Semantic Recovery](backlog/VM-517-white-semantic-recovery.md) - governance hold after drift preflight; `STOP - WHITE GATE 1+2 NOT AUTHORIZED` because `--identity=W` candidate-scope validation crashes on White's monocolor `collision_guidance` object-with-`pairs` shape; no White source inspection, Gate 1+2 audit, remediation, candidate, review, certification, or VM-518 work started.
