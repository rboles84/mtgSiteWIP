...
- [VM-526 - Naya Semantic Recovery](in-progress/VM-526-naya-semantic-recovery.md) - Gate 1+2 passed from exact preflight HEAD `813c42c63a56648029c7452d2619cdaf60432b5a`; `NAYA` confirmed as canonical key, `WRG`/`RGW`/`GRW` remain metadata-only invalid identity aliases; Gate 3+4 remediation is authorized for NAYA only, while candidate/review/certification remain unstarted.
