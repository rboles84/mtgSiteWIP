...
- [VM-528 - Temur Semantic Recovery](in-progress/VM-528-temur-semantic-recovery.md) - Gate 1+2 read-only audit is complete from drift-preflight commit `20c9413f39273bf76a11c4fdddb2163dd61c8037`; Gate 3+4 remediation is authorized for `TEMUR` only. `GUR`, `URG`, and `RGU` remain invalid identity aliases. No semantic candidate, review, certification, Excel edit, VM-529 work, push, PR, or merge has occurred.
