...
- [VM-551 — Gate A Archscry Trust Containment](in-progress/VM-551-gate-a-trust-containment-design.md) - final bounded owner-correction candidate ready for review: the original tied hero is first; compact co-leader context lives in the existing snapshot; every 390px/320px directory tab reveals its panel; public explanations use answer/signal/reading language; Matrix/precon rhythm is tightened; and card names/images share canonical Scryfall links plus the established preview path. Focused product, cache, placement, lint, copy, route, deck, Maze, and four-viewport browser checks pass. Accepted scoring, cache, downstream, Matrix-value, recommendation, deck-link, and Maze contracts remain unchanged; Gate B1, merge, push, deployment, and certification remain excluded.
