...
- [VM-551 — Gate A Archscry Trust Containment](in-progress/VM-551-gate-a-trust-containment-design.md) - final two-blocker correction candidate ready for owner review: ordinary mobile tab clicks no longer receive unconditional pointer capture, drag suppression clears with its own pointer sequence, and one delegated action opens each panel exactly once; card previews are image/immediate-image-link only, caption-free, and close on image leave, scroll, or panel change; compact co-leader pips use a tighter local gap. Focused product/cache/placement/lint/copy/route/deck/Maze and real-pointer 1440/820/390/320 browser checks pass. Accepted state, scoring, cache, Matrix-value, precon, recommendation, deck-link, and Maze behavior remains protected; Gate B1, merge, push, deployment, and certification remain excluded.
