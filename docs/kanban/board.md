...
- [VM-516 - Simic Semantic Recovery](backlog/VM-516-simic-semantic-recovery.md) - setup only after VM-515 certification; Gate 1+2 pending and blocked until the CRIT-001 drift-control baseline is installed, the external tracker is updated, and a separate VM-516 drift-preflight control record passes; no Simic source inspection, audit, remediation, candidate, review, or certification started.
