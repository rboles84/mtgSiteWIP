...
| Boros Legion | WR | `data/raw-factions/boros_legion/` | 17 | `claim-bearing`: 5; `discovery-only`: 12 | 0 | Official guide, novel, Boros lore article, Karlov Manor legends article, and mechanics article cover current claim-bearing floor. | Semantically ready under CRIT-001 Contract v1.1 via VM-509; approved recovery SHA `da2e9ef4036c427c17dca66c5a1a9d9a8fe03436`. | Commander/deck support remains auxiliary only; story-corpus rows stay discovery-only and are not authoritative key-figure proof. | Non-blocking: future official page capture could replace or strengthen novel/story archive dependencies where exact public-source proof is desired; review-sandbox dossier-audit artifact-write limitation required no candidate correction. |
