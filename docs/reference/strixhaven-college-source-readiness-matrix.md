...
Created: 2026-06-12
Related cards: VM-343, VM-345, VM-346, VM-349, VM-351, VM-353, VM-357
...
| Prismari College | PRISMARI | `data/raw-factions/prismari/` | 13 | `claim-bearing`: 3; `discovery-only`: 10 | 0 | Official 2021 guide, product page, and 2026 guide cover elemental arts, magic-as-art, technique/expression tension, study areas, and placement signals. Story-corpus rows remain discovery-only. | Ready after VM-346. Added `prismari_q3` for Prismari-vs-Quandrix and `prismari_q4` for Prismari-vs-Silverquill, plus paired collision guidance for both pairs. Enabled `crucible_PRISMARI_QUANDRIX` and `crucible_PRISMARI_SILVERQUILL`. | No new Commander recommendations were added. Existing deck/display copy should remain support texture unless tied to claim-bearing rows. | Future story deep-read can promote specific Rootha/Uvilda/performance claims if source-read and rebound; do not promote search hits by name alone. |
| Quandrix College | QUANDRIX | `data/raw-factions/quandrix/` | 13 | `claim-bearing`: 3; `discovery-only`: 10 | 0 | Official 2021 guide, product page, and 2026 guide cover mathematics, patterns, fractals, proof/model logic, study areas, and placement signals. Story-corpus rows remain discovery-only. | Ready after VM-346. Existing Simic/Izzet collisions remain. Added paired collisions for Prismari, Lorehold, and Witherbloom. Enabled `crucible_PRISMARI_QUANDRIX`, `crucible_LOREHOLD_QUANDRIX`, and `crucible_QUANDRIX_WITHERBLOOM`. | No new Commander recommendations were added. Commander/deck support remains display support only unless later tied to claim-bearing rows. | Future metaphysics work should stay explicit about what is official study-area language versus Vox Mana placement interpretation. |
| Silverquill College | SILVERQUILL | `data/raw-factions/silverquill/` | 13 | `claim-bearing`: 3; `discovery-only`: 10 | 0 | Official 2021 guide, product page, and 2026 guide cover eloquence, word magic, speech/writing/signing, reputation, leadership, study areas, and placement signals. Story-corpus rows remain discovery-only. | Ready after VM-346. Existing Orzhov/Dimir collisions remain. Added paired collision guidance for Prismari-vs-Silverquill and enabled `crucible_PRISMARI_SILVERQUILL`. | No new Commander recommendations were added. Existing politics/rhetoric deck support should remain support texture unless later bound to claim-bearing rows. | Future story deep-read can promote specific Silverquill character or school-politics claims if source-read and rebound. |
| Witherbloom College | WITHERBLOOM | `data/raw-factions/witherbloom/` | 13 | `claim-bearing`: 3; `discovery-only`: 10 | 0 | Official 2021 guide, product page, and 2026 guide cover essence studies, life/death exchange, practical components, fieldwork, remedies/poisons, study areas, and placement signals. Story-corpus rows remain discovery-only. | Ready after VM-346. Existing Selesnya/Simic collisions remain. Added paired collision guidance for Quandrix-vs-Witherbloom and enabled `crucible_QUANDRIX_WITHERBLOOM`. | No new Commander recommendations were added. Existing deck support remains display support unless later tied to claim-bearing rows. | Future story deep-read can promote specific Dina/Willowdusk/fieldwork claims only after source-reading and claim binding. |

## Cohort Totals

- Total active source rows: 72.
- `claim-bearing`: 25.
- `support-only`: 0.
- `shaping-only`: 0.
- `discovery-only`: 47.
...
| Prismari, Quandrix, Silverquill, and Witherbloom figure/flavor enrichment | `source-intake-needed` | Future source-ledger promotion of currently discovery-only story corpus rows | Existing raw profile figure/flavor notes are not enough for public raw-enrichment parity until their source role is promoted. |
| Non-Lorehold generated/public `raw_enrichment` surfacing | `blocked-noncanonical` | None until field-specific backing exists | VM-357 must suppress stale generated/runtime raw-enrichment output through canonical builder gates unless exact rows are promoted. |
| Non-Lorehold raw-enrichment builder allowlist | `blocked-noncanonical` | None until field-specific backing exists | Do not add Prismari/Quandrix/Silverquill/Witherbloom to raw-enrichment generation from generated/runtime parity pressure. |
| Non-Lorehold deck links and research links | `source-intake-needed` | Approved Commander recommendation or product-support rows | Existing generated/public deck or research metadata is not preservation evidence; suppress until approved rows authorize the public surface. |
| Non-Lorehold Commander Compass expansion | `source-intake-needed` | Approved Commander recommendation or product-support rows | Existing raw-profile or generated Compass data is not enough for public surfacing until source-role review authorizes it. |
