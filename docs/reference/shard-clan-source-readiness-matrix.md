...
Related cards: `VM-347 - Shard And Tarkir Clan Source-Bound Cohort Repair`, VM-349, VM-352, VM-353, VM-358
...
| `GRIXIS` | Full: 2 `claim-bearing`, 7 `shaping-only`, 2 `support-only` | 3 | 2 | Existing support-only Compass retained. | VM-352 adds one backed Dimir/Rakdos/Izzet false-positive discriminator. |
...
| `ABZAN` | Full: 9 `claim-bearing`, 6 `shaping-only`, 4 `support-only` | 3 | 2 | Support-only Commander rows available; no new top-level deck links. | VM-352 adds one backed generic WBG/Dromoka/Commander-product/graveyard/toughness false-positive discriminator. |
| `TEMUR` | Full: 9 `claim-bearing`, 8 `shaping-only`, 3 `support-only` | 3 | 3 | Support-only Commander rows available; no new top-level deck links. | Added Sultai/Mardu/Jeskai discriminator and collision guidance. |
| `SULTAI` | Full: 7 `claim-bearing`, 7 `shaping-only`, 3 `support-only` | 3 | 3 | Support-only Commander rows available; no new top-level deck links. | Added Abzan/Temur/Jeskai discriminator and collision guidance. |
| `MARDU` | Full: 8 `claim-bearing`, 8 `shaping-only`, 2 `support-only` | 3 | 0 | Support-only Commander Compass filled from local JSONL rows; top-level `research_links`/`deck_links` still empty. | Normalized VM-228 lifecycle source role to `shaping-only`; discriminator content unchanged. |
| `JESKAI` | Full: 7 `claim-bearing`, 8 `shaping-only`, 2 `support-only`, 3 `discovery-only` | 3 | 3 | Support-only Commander Compass filled from local JSONL rows; top-level `research_links`/`deck_links` still empty. | Normalized VM-234/manual-fill/seed roles; repaired non-array collision guidance shape. |
...
- Top-level Mardu/Jeskai `research_links` and `deck_links` remain empty; VM-347 filled Compass `link_targets` only, using support-only Commander/operator rows.
...
| Esper/Grixis/Jund/Naya figure and flavor expansion | `source-intake-needed` | Explicit raw/research backing per field | Do not fill from general MTG knowledge, generated display, snippets, or runtime copy. |
| Abzan/Temur/Sultai Commander Compass fields | `source-intake-needed` | Approved Commander recommendation/product-support rows with public-surface permission | Existing support-only rows are not enough to add broad public Compass fields in VM-353. |
| Mardu/Jeskai top-level deck links and research links | `source-intake-needed` | Approved Commander/source rows that authorize top-level dossier surfacing | Keep current support-only `link_targets` boundary unless later intake promotes source rows. |
| Existing Mardu/Jeskai Commander Compass link targets | `source-normalization` | Existing support-only Commander/operator rows | Preserve current Compass `link_targets` unless source-role review shows a failure; do not broaden into top-level public navigation. |
