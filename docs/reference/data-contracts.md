...
`data/dossier/card-voice-printings.source.json` remains the exact-printing authority for public approved records. Proposal records carry their exact printing inline during owner review and do not enter this approved printing source until a separate promotion pass is authorized. The producer `research/apply-vm551-card-content-automatic-approval.mjs` promotes only structurally valid slot-2 records with an explicit owner `APPROVE`, projects their exact printings into the approved printing source, preserves unresolved review-required records as non-public, and emits only `APPROVED_PUBLIC` relationships to `data/dossier/card-voice-catalog.json`.
