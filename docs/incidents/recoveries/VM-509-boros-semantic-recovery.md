...
Candidate-scope cleanup applied before replacement candidate creation:

- Preserved Gate 3 canonical remediation, Gate 4 generated artifacts/provenance, Boros semantic fixtures, collision target preservation, and Boros display-source/public-copy cleanup.
- Preserved all 8 generated WR collision guidance entries: `WR`, `WU`, `LOREHOLD`, `WG`, `BR`, `UR`, `WB`, and `MARDU`.
- Restored generated WR lateral-inhibition targets to the parent-stable set: `WU`, `LOREHOLD`, `WG`, and `MARDU`.
- Preserved explicit `lateral_inhibition: false` as a non-inhibiting opt-out; no lateral-inhibition behavior was added.
- Restored frozen placement scoring hints at `placement_axes/0/scoring_hint` and `placement_axes/2/scoring_hint` to the accepted parent values.
- Restored native IDs exactly once as metadata-only retained IDs under `data_quality.corpus_upgrade.retained_native_ids`:
  - `axis_id:axis_boros_righteous_zeal`
  - `character_id:char_tajic`
- Did not revive `axis_boros_righteous_zeal` as the active placement axis.
- Did not promote Tajic back into authoritative key figures.
- Confirmed stale high-heat public copy remains absent.
...
## Corrected Replacement Candidate Review Fixes

Independent review requested changes for rejected replacement candidate `c2f5d064460a007f0dca6be95b7beabb4ca85026` and workflow record `d7f2523d53cfbc6420e75c83b9ab03192158a1a1`. The rejected candidate is not being sent back to review.

Corrections applied for the new corrected replacement candidate:

- Cleaned WR/Boros generated-display key-figure chains in `data/factions.json#/factions/WR/raw_enrichment/key_figures` so discovery records are not presented as source-backed faction proof.
- Removed Tajic from authoritative generated Boros key figures; `character_id:char_tajic` remains retained only as native-ID metadata, not semantic proof.
- Preserved Aurelia, Razia, Feather, and Agrus Kos generated key-figure chains only with substantive claim IDs.
- Corrected `boros_legion_claim_026` Gatecrash evidence locator from the mismatched "A Flavorful Guide to the Guilds of Ravnica - Boros Legion reviewed source record" text to "The Boros Legion - reviewed source record" without changing claim meaning.
- Rebuilt generated artifacts and semantic provenance after the corrections.
- Confirmed generated WR lateral targets remain parent-stable: `WU`, `LOREHOLD`, `WG`, `MARDU`.
- Confirmed generated WR collision guidance still preserves all 8 required neighbors: `WR`, `WU`, `LOREHOLD`, `WG`, `BR`, `UR`, `WB`, `MARDU`.
