...
Vox Mana is an unofficial Magic: The Gathering fan site that combines a themed static frontend, an adaptive faction-placement engine, a Scryfall research workspace, generated lore data, and a Supabase edge function for the deeper interview path.
...
| `/archscry/` | `archscry/index.html` | Placement experience: landing, quick adaptive reading, Scrying Terminal, dossier result. | `assets/js/index.js`, `assets/js/shared.js`, `assets/js/adaptive-placement.js` |
...
| Placement frontend | `assets/js/index.js`, `assets/js/adaptive-placement.js`, `assets/js/shared.js` | Loads data, runs quick adaptive reading, calls interview API, renders dossiers, saves and resumes results. |
...
| Backend interview | `supabase/functions/guild-recruiter/index.ts` | Deno edge function for Anthropic-backed Scrying Terminal turns and normalized placement decisions. |
...
| `supabase/functions/guild-recruiter/index.ts` | Deno edge function | Handles `OPTIONS` and `POST`, enforces request limits, calls Anthropic, and normalizes model output. |
...
| Anthropic API call | `supabase/functions/guild-recruiter/index.ts` | Messages endpoint with prompt built from generated faction context. |
...
|---|---|---|
| Faction artifacts | `npm run build:factions` from `C:\dev\projectFiles\voxmana-tools` | Rewrites generated faction/model/schema/context artifacts in this repo. |
