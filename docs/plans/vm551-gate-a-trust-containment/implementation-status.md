...
Final owner spot-check follow-up: the optional commander-preview grid no longer exposes a redundant `Commander starting points` label or an empty decorated block. The block begins hidden, becomes visible only when card validation marks at least one commander preview as verified, and is removed when none resolve. In tied snapshots, the co-leader name and Mana Font span share one flex header row; the span is scoped to `width: max-content`, left alignment, and zero internal gap. Symbol generation, order, size, glow, and accessible labeling are unchanged.
