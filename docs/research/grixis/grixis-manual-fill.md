...
Rule: if a downstream sentence depends on one of these rows, label it `Manual fill required` or add a stronger local evidence row first.

---

## Manual Fill Claims

| ID | Claim / Topic | Why It Is Manual Fill Required | Minimum Evidence Needed |
|---|---|---|---|
| GRIXIS-MF-001 | Current official Wizards plane-page wording for Grixis as an undead hellscape and post-Conflux aggressor | The drafts cite external official pages, but VM-164 does not have a local official capture in the approved packet | Local official capture or approved excerpt notes |
...
## Claims That Must Be Labeled Vox Mana Synthesis

These may be useful in VM-165 but are not MTG canon:

| Synthesis Topic | Required Label | Evidence Support |
|---|---|---|
| "Survival through exploitation, calculation, and volatility" | `Vox Mana synthesis` | GRIXIS-003 through GRIXIS-017 |
| "Black uses Blue's method and Red's urgency as tools" | `Vox Mana synthesis` unless phrased only as Rosewater design paraphrase | GRIXIS-004 through GRIXIS-006 |
| "Grixis Commander experience rewards graveyard pressure, spells, sacrifice, forced choice, cascade, and ruthless value" | `Vox Mana synthesis` or support-only product language | GRIXIS-016, GRIXIS-017 |
| "Unearth feels like borrowed life" | `Vox Mana synthesis` unless tied only to card mechanics | GRIXIS-011 |

---

## Comparator-Only Material

| Comparator | Allowed Use | Not Allowed |
|---|---|---|
| Maestros / New Capenna UBR | Same-color-direction contrast; product row for Anhelo/Parnesse; reminder that UBR is not automatically Grixis | Evidence for Grixis lore, Grixis social logic, or Grixis aesthetics |
| Dimir | Future comparator for shared UB secrecy/knowledge/ambition | Evidence that Grixis is only Dimir plus Red |
| Rakdos | Future comparator for shared BR appetite/violence/survival | Evidence that Grixis is only Rakdos plus Blue |
| Izzet | Future comparator for shared UR experimentation/volatility | Evidence that Grixis is only Izzet plus Black |
| Esper | Future Alara/UB contrast for control/perfection versus survival/exploitation | Evidence for positive Grixis doctrine |
| Jund | Future Alara survival contrast | Evidence for Grixis ecology unless source-bound |
| Sultai | Future Black-centered exploitation/resource hierarchy contrast | Evidence that all Black-centered three-color identities behave the same |
| Bant | Future opposite-world order/honor/community contrast | Evidence for Grixis beyond absence contrast |

---

## Stop Rule

Do not move a manual-fill row into architecture, raw JSON, generated data, placement copy, or runtime behavior unless a future card adds direct local evidence and updates [grixis-evidence-ledger.md](grixis-evidence-ledger.md).
