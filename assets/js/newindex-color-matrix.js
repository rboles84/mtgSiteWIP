(function () {
  ...
  const identities = [
    ...Object.values(colorProfiles).map(function (color) {
      return {
        id: color.key,
        type: "mono",
        group: "mono",
  ...
  function valueToResonanceTier(value) {
    const tier = resonanceTiers.find(function (candidate) {
      return value <= candidate.max;
    });
  ...
  function blendGradient(hexes) {
    ...
    const stops = hexes.map(function (hex, index) {
      const positions = ["35% 38%", "65% 62%", "50% 50%"];
  ...
  function radialFill(chart, hexes) {
    if (!chart || !chart.chartArea) {
      return hexToRgba((hexes && hexes[0]) || "#ffffff", 0.12);
    }
  ...
  function manaSymbols(code) {
    return code.split("").map(function (symbol) {
      return "{" + symbol + "}";
    }).join("");
  }
  ...
  function findIdentity(id) {
    return identities.find(function (identity) {
      return identity.id === id || identity.key === id;
    });
  }
  ...
  function renderAxisList(identity) {
    ...
    axisList.innerHTML = axisLabels.map(function (label, index) {
  ...
  function buildChart() {
    ...
    return new window.Chart(radarContext, {
          ...
          tooltip: {
            callbacks: {
              label: function (context) {
                const value = context.raw !== undefined ? context.raw : (context.parsed && context.parsed.r);
                const tier = valueToResonanceTier(Number(value));
  ...
  function buildDatasets(identity) {
    ...
    const showComponents = componentToggle ? componentToggle.checked : true;
    const showSynthesis = compositeToggle ? compositeToggle.checked : true;

    if (identity.components.length > 1 && showComponents) {
      identity.components.forEach(function (componentKey) {
    ...
    if (showSynthesis || identity.components.length === 1) {
      datasets.push({
        label: identity.name,
        data: identity.data,
        backgroundColor: identity.components.length > 1
          ? radialFill(radarChart, identity.components.map(function (key) {
              return colorProfiles[key].hex;
            }))
  ...
  function updateGlow(identity) {
    ...
    const glowHexes = identity.components.length > 1
      ? identity.components.map(function (key) {
          return colorProfiles[key].hex;
        })
      : [identity.hex];

    glow.style.background = blendGradient(glowHexes);
  }
  ...
  function getVisibleIdentities() {
    if (lensState.mode === "mono") {
      return identities.filter(function (identity) {
        return identity.group === "mono";
      });
    }

    if (lensState.mode === "guilds") {
      return identities.filter(function (identity) {
        return identity.group === lensState.guildGroup;
      });
    }

    return identities.filter(function (identity) {
      return identity.group === "strixhaven";
    });
  }
  ...
  function renderIdentityGrid() {
    ...
    grid.innerHTML = visibleIdentities.map(function (identity) {
      const description = identity.components.length > 1
        ? identity.components.map(function (key) {
            return colorProfiles[key].name;
          }).join(" + ")
        : "Core color profile";

      return (
        '<button class="vm-lens-choice ' + (identity.id === lensState.selectedId ? "active" : "") + '"' +
          ' data-id="' + identity.id + '"' +
          ' style="--choice-glow:' + blendChoiceGlow(identity) + '">' +
          '<span class="vm-lens-symbols">' + manaSymbols(identity.code) + "</span>" +
          '<span class="vm-lens-name">' + identity.name + "</span>" +
          '<span class="vm-lens-mini">' + description + "</span>" +
        "</button>"
      );
    }).join("");

    grid.querySelectorAll(".vm-lens-choice").forEach(function (choice) {
      choice.addEventListener("click", function () {
        updateRadar(choice.dataset.id);
      });
    });
  }

  function updateSelectedCard(identity) {
    const kicker = document.getElementById("selectedKicker");
    ...
    if (kicker) {
      kicker.textContent = identity.components.length > 1 ? "Selected Synthesis" : "Selected Profile";
    }
    ...
    if (text) {
      text.textContent = identity.text;
    }
    ...
    const componentDots = identity.components.map(function (key) {
  ...
  function updateActiveLensChoice(identity) {
    ...
    document.querySelectorAll(".vm-lens-choice").forEach(function (choice) {
      choice.classList.toggle("active", choice.dataset.id === identity.id);
    });
  }

  function updateDatasetPills(datasets, identity) {
    const pills = document.getElementById("datasetPills");
    if (!pills) {
      return;
    }
    ...
    pills.innerHTML = datasets.map(function (dataset) {
      ...
      return (
        '<span class="vm-pill' +
          ...
          ' style="border-color:' + dataset.borderColor + ';">' +
          dataset.label +
        "</span>"
      );
    }).join("");
  ...
  function updateRadar(identityId, refreshGrid) {
    ...
    if (caption) {
      caption.textContent = "Current profile: " + identity.name;
    }

    if (radarChart) {
      radarChart.options.animation = chartAnimationOptions();
      radarChart.data.datasets = datasets;
      radarChart.update(reducedMotionEnabled() ? "none" : undefined);
    }
  ...
  function whenMatrixVisible(callback) {
    window.requestAnimationFrame(function () {
      window.requestAnimationFrame(callback);
    });
  }

  function ensureRadarReady(forceRecreate) {
    const matrix = document.getElementById("colorMatrixWrap");
    const caption = document.getElementById("radarCaption");
    ...
    if (!window.Chart) {
      if (caption) {
        caption.textContent = "Chart.js is unavailable for this preview.";
      }
      return false;
    }

    if (forceRecreate && radarChart) {
      radarChart.destroy();
      radarChart = null;
    }

    if (!radarChart) {
      radarChart = buildChart();
      if (!radarChart) {
        return false;
      }
    }

    whenMatrixVisible(function () {
      if (!radarChart) {
        return;
      }
      radarChart.resize();
  ...
  function setLensMode(mode) {
    ...
    document.querySelectorAll(".vm-lens-tab").forEach(function (tab) {
    ...
    const visibleIdentities = getVisibleIdentities();
    const currentStillVisible = visibleIdentities.some(function (identity) {
      return identity.id === lensState.selectedId;
    });

    if (!currentStillVisible && visibleIdentities.length) {
      lensState.selectedId = visibleIdentities[0].id;
    }

    renderIdentityGrid();
    ensureRadarReady(false);
    updateRadar(lensState.selectedId, false);
  }
  ...
  function setGuildGroup(group) {
    ...
    document.querySelectorAll(".vm-lens-subtab").forEach(function (tab) {
  ...
  function bindControls() {
    ...
    if (componentToggle) {
      componentToggle.addEventListener("change", function () {
        ensureRadarReady(false);
        updateRadar(lensState.selectedId, false);
      });
    }
    ...
    document.querySelectorAll(".vm-lens-tab").forEach(function (tab) {
      tab.addEventListener("click", function () {
        setLensMode(tab.dataset.mode);
      });
    });

    document.querySelectorAll(".vm-lens-subtab").forEach(function (tab) {
      tab.addEventListener("click", function () {
        setGuildGroup(tab.dataset.guildGroup);
      });
    });
  }

  function watchMatrixVisibility() {
    const matrix = document.getElementById("colorMatrixWrap");
    if (!matrix || !window.MutationObserver) {
      return;
    }
  ...
  function init() {
    if (!document.getElementById("identityGrid") || !document.getElementById("vmRadar")) {
      return;
    }

    renderIdentityGrid();
    bindControls();
    watchMatrixVisibility();
    watchMotionChanges();
    updateRadar(lensState.selectedId, false);
    ensureRadarReady(false);
  }
